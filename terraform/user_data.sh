...
###############################################################################
# user_data.sh
# Inicializa a EC2: ativa o agente SSM, instala Docker e executa o container.
# Variáveis interpoladas pelo Terraform (templatefile):
#   ${frontend_image}, ${container_port}, ${aws_region}, ${db_secret_arn}
###############################################################################
 
# Garante o agente do Systems Manager ATIVO logo no início, antes de qualquer
# "set -e". Assim a instância registra no Session Manager mesmo que algum passo
# abaixo falhe. (Na AL2023 o pacote existe, mas pode não estar habilitado.)
